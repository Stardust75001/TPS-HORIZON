...
function displayDefaultBanner() {
  ...
  if (banner) {
    ...
    console.warn('privacy-banner element not found in DOM.');
  }
}
...
function sendToMonorail(data) {
  try {
    if (isValidData(data)) {
      ...
      console.warn("⛔ Invalid consent data. Not sent.");
    }
  } catch (err) {
...
if (!customElements.get(elementName)) {
  try {
    ...
    console.warn(\`Failed to define \${elementName}:\`, error);
    try {
  ...
  console.log(\`\${elementName} is already defined.\`);
}
  ...
  .then(data => {
    ...
    const eventData = {
      ...
      payload: {
        ...
        shop_id: window.Shopify && Shopify.shop || 'unknown'
      }
    };
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".animate__animated.opacity-0").forEach(el => {
    if (el.dataset.animateClass && el.dataset.animateClass.trim() !== '') {
